...
logger = logging.getLogger("PPEAgent.Alerts")


class PPEAlerts:
    """PPE Compliance Detection Alerts Component."""
    ...
    def __init__(self, agent_id: str, config: Dict[str, Any], mqtt_client: Optional[Any] = None):
        ...
        logger.info(f"Initialized PPE Alerts (channels: {len(config.get('channels', []))})")
    ...
    async def handle_alerts(self, logic_output: Dict[str, Any]) -> None:
        ...
        try:
            ...
            alert = {
                ...
                "data": {
                    "compliance_rate": logic_output.get('compliance_rate'),
                    "violations": logic_output.get('violations', []),
                    "violations_count": logic_output.get('violations_count', 0),
                    "action": logic_output.get('action'),
                    "access_denied": logic_output.get('access_denied', False)
                }
            }
    ...
    def _build_alert_message(self, logic_output: Dict[str, Any]) -> str:
        ...
        compliance_rate = logic_output.get('compliance_rate', 0.0)
        violations = logic_output.get('violations', [])
        access_denied = logic_output.get('access_denied', False)
        
        msg = f"[{alert_level}] PPE Compliance Alert - Compliance: {compliance_rate:.1f}%"
        if violations:
