...
logger = logging.getLogger("PMAgent.State")


class PMState:
    """
    Predictive Maintenance State Component.
    ...
    def __init__(self, agent_id: str, buffer_size: int = 100, checkpoint_interval: int = 300,
                 checkpoint_path: str = "state/pm_agent_state.json"):
        """
        ...
        logger.info(f"Initialized PM State (buffer_size={buffer_size}, checkpoint_interval={checkpoint_interval}s)")
    ...
    def update(self, prediction: Dict[str, Any], logic_output: Dict[str, Any], success: bool = True) -> None:
        ...
        if alert_level == 'CRITICAL' or alert_level == 'EMERGENCY':
            self.state_manager.custom_state['consecutive_critical'] += 1
        else:
            self.state_manager.custom_state['consecutive_critical'] = 0
