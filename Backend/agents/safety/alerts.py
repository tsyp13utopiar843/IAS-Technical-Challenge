...
logger = logging.getLogger("HazardAgent.Alerts")


class HazardAlerts:
    """Workplace Hazard Detection Alerts Component."""
    ...
    def __init__(self, agent_id: str, config: Dict[str, Any], mqtt_client: Optional[Any] = None):
        ...
        logger.info(f"Initialized Hazard Alerts (channels: {len(config.get('channels', []))})")
    ...
    async def handle_alerts(self, logic_output: Dict[str, Any]) -> None:
        ...
        try:
            ...
            alert = {
                ...
                "data": {
                    "hazard_score": logic_output.get('hazard_score'),
                    "hazard_type": logic_output.get('hazard_type'),
                    "safety_score": logic_output.get('safety_score'),
                    "action": logic_output.get('action'),
                    "evacuation_required": logic_output.get('evacuation_required', False)
                }
            }
    ...
    def _build_alert_message(self, logic_output: Dict[str, Any]) -> str:
        ...
        safety_score = logic_output.get('safety_score', 100.0)
        evacuation = logic_output.get('evacuation_required', False)
        
        msg = f"[{alert_level}] Workplace Hazard Alert - Type: {hazard_type}, Safety: {safety_score:.1f}%"
        if evacuation:
            msg += " - EVACUATION REQUIRED"
        return msg
