"""
Hazard Agent Main Orchestrator
Workplace Hazard Detection Agent - Main entry point and orchestrator.
"""
import asyncio
import logging
import signal
...
from agents.safety.model import HazardModel
from agents.safety.logic import HazardLogic
from agents.safety.state import HazardState
from agents.safety.communication import HazardCommunication
from agents.safety.alerts import HazardAlerts

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger("HazardAgent")


class HazardAgent(BaseAgent):
    """Workplace Hazard Detection Agent."""
    
    def __init__(self, config: Dict[str, Any]):
        agent_id = config.get('agent', {}).get('id', 'hazard_agent')
        super().__init__(agent_id, config)
        self._init_components()
        logger.info(f"Hazard Agent initialized: {agent_id}")
    
    def _init_components(self) -> None:
        model_config = self.config.get('model', {})
        self.model = HazardModel(
            model_path=model_config.get('path', 'artifacts/hazard_model.pkl'),
            num_features=model_config.get('num_features', 6),
            scaler_path=model_config.get('scaler_path'),
            label_encoders_path=model_config.get('label_encoders_path')
        )
        
        logic_config = self.config.get('logic', {})
        self.logic = HazardLogic(logic_config.get('thresholds', {}))
        ...
        self.state = HazardState(
            ...
            checkpoint_path=state_config.get('checkpoint_path', 'state/hazard_agent_state.json')
        )
        
        comm_config = self.config.get('communication', {})
        self.communication = HazardCommunication(
            agent_id=self.agent_id,
            config=comm_config,
    ...
    async def _process_sensor_data(self, sensor_data: Dict[str, Any]) -> None:
        try:
            ...
            logger.info(f"Prediction: Hazard={prediction.get('hazard_score')}, "
                       f"Type={prediction.get('hazard_type')}, Safety={prediction.get('safety_score')}%")
    ...
    async def initialize(self) -> None:
        ...
        logger.info("Hazard Agent initialized successfully")
...
async def main():
    ...
    config = load_agent_config('hazard_agent', config_dir)
    
    safety_dir = os.path.dirname(__file__)
    backend_root = os.path.dirname(os.path.dirname(safety_dir))
    
    if 'model' in config:
        model_path = config['model'].get('path', 'artifacts/hazard_model.pkl')
        if not os.path.isabs(model_path):
            full_path = os.path.join(safety_dir, model_path)
        ...
        if scaler_path and not os.path.isabs(scaler_path):
            full_path = os.path.join(safety_dir, scaler_path)
            if not os.path.exists(full_path):
                full_path = os.path.join(backend_root, scaler_path.replace('../', ''))
            config['model']['scaler_path'] = full_path
        
        label_encoders_path = config['model'].get('label_encoders_path')
        if label_encoders_path and not os.path.isabs(label_encoders_path):
            full_path = os.path.join(safety_dir, label_encoders_path)
            if not os.path.exists(full_path):
                full_path = os.path.join(backend_root, label_encoders_path.replace('../', ''))
            config['model']['label_encoders_path'] = full_path
    
    if 'state' in config:
        state_path = config['state'].get('checkpoint_path', 'state/hazard_agent_state.json')
        if not os.path.isabs(state_path):
            state_dir = os.path.join(safety_dir, 'state')
            os.makedirs(state_dir, exist_ok=True)
            config['state']['checkpoint_path'] = os.path.join(state_dir, 'hazard_agent_state.json')
    ...
    try:
        await agent.start()
        logger.info("Hazard Agent started. Press Ctrl+C to stop.")
        try:
        ...
        await agent.stop()
        logger.info("Hazard Agent stopped")
