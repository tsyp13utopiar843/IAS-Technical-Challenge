"""
Hazard Agent Communication Component
...
logger = logging.getLogger("HazardAgent.Communication")


class HazardCommunication:
    """Workplace Hazard Detection Communication Component."""
    ...
    def __init__(self, agent_id: str, config: Dict[str, Any], 
        ...
        self.publish_topic = mqtt_config.get('publish_topics', ['predictions/hazard_agent'])[0]
        
        self.app = FastAPI(title=f"{agent_id} API")
        self.api_port = config.get('api', {}).get('port', 8004)
        ...
        self._setup_routes()
        logger.info(f"Initialized Hazard Communication (MQTT: {broker}:{port}, API: {self.api_port})")
        ...
        try:
            message = {
                ...
                "prediction": {
                    ...
                    "safety_score": prediction.get('safety_score'),
                    "alert_level": prediction.get('alert_level'),
                    "action": prediction.get('action'),
                    "evacuation_required": prediction.get('evacuation_required', False)
                }
            }
