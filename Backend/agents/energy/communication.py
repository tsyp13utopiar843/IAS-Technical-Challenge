"""
Energy Agent Communication Component
...
logger = logging.getLogger("EnergyAgent.Communication")


class EnergyCommunication:
    """
    Energy Consumption Optimization Communication Component.
    ...
    def __init__(self, agent_id: str, config: Dict[str, Any], 
        ...
        self.publish_topic = mqtt_config.get('publish_topics', ['predictions/energy_agent'])[0]
        
        # FastAPI app
        self.app = FastAPI(title=f"{agent_id} API")
        self.api_port = config.get('api', {}).get('port', 8002)
        ...
        logger.info(f"Initialized Energy Communication (MQTT: {broker}:{port}, API: {self.api_port})")
    ...
    async def publish_prediction(self, prediction: Dict[str, Any]) -> None:
        ...
        try:
            message = {
                ...
                "prediction": {
                    "consumption_kwh": prediction.get('consumption_kwh'),
                    "efficiency_score": prediction.get('efficiency_score'),
                    "anomaly_score": prediction.get('anomaly_score'),
                    "is_anomaly": prediction.get('is_anomaly'),
                    "baseline_consumption": prediction.get('baseline_consumption'),
                    "alert_level": prediction.get('alert_level'),
                    "action": prediction.get('action'),
                    "optimization_recommendations": prediction.get('optimization_recommendations', [])
                }
            }
