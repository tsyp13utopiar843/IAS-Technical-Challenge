...
logger = logging.getLogger("EnergyAgent.State")


class EnergyState:
    """
    Energy Consumption Optimization State Component.
    
    Manages agent state, prediction history, metrics, and checkpointing.
    """
    
    def __init__(self, agent_id: str, buffer_size: int = 100, checkpoint_interval: int = 300,
                 checkpoint_path: str = "state/energy_agent_state.json"):
        """
        ...
        logger.info(f"Initialized Energy State (buffer_size={buffer_size}, checkpoint_interval={checkpoint_interval}s)")
    ...
    def update(self, prediction: Dict[str, Any], logic_output: Dict[str, Any], success: bool = True) -> None:
        ...
        """
        # Combine prediction and logic output
        combined = {
            **prediction,
            **logic_output,
        ...
        # Update energy-specific state
        alert_level = logic_output.get('alert_level', 'NORMAL')
        self.state_manager.custom_state['last_alert_level'] = alert_level
        
        # Track baseline
        if 'baseline_consumption' in prediction:
            self.state_manager.custom_state['baseline_consumption'] = prediction['baseline_consumption']
        
        # Track optimizations
        if alert_level in ['WARNING', 'CRITICAL']:
            self.state_manager.custom_state['optimization_count'] += 1
    ...
    def save_state(self, filepath: Optional[str] = None) -> None:
        ...
        Args:
            filepath: Optional override path (uses checkpoint_path if None)
        """
        try:
