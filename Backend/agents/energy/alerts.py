...
logger = logging.getLogger("EnergyAgent.Alerts")


class EnergyAlerts:
    """Energy Consumption Optimization Alerts Component."""
    ...
    def __init__(self, agent_id: str, config: Dict[str, Any], mqtt_client: Optional[Any] = None):
        ...
        logger.info(f"Initialized Energy Alerts (channels: {len(config.get('channels', []))})")
    ...
    async def handle_alerts(self, logic_output: Dict[str, Any]) -> None:
        ...
        try:
            ...
            alert = {
                ...
                "data": {
                    "consumption_kwh": logic_output.get('consumption_kwh'),
                    "efficiency_score": logic_output.get('efficiency_score'),
                    "anomaly_score": logic_output.get('anomaly_score'),
                    "action": logic_output.get('action'),
                    "optimization_recommendations": logic_output.get('optimization_recommendations', [])
                }
            }
    ...
    def _build_alert_message(self, logic_output: Dict[str, Any]) -> str:
        ...
        efficiency = logic_output.get('efficiency_score', 0.0)
        consumption = logic_output.get('consumption_kwh', 0.0)
        action = logic_output.get('action', 'UNKNOWN')
        
        return (f"[{alert_level}] Energy Optimization Alert - "
                f"Efficiency: {efficiency:.1f}%, Consumption: {consumption:.1f} kWh, Action: {action}")
